...
func main() {
	inFilename, outFilename, err := filenamesFromCommandLin()
	if err != nil {
		fmt.Println(err)
		os.Exit(1)
	}
	...
	if err = americanise(inFile, outFile); err != nil { 			log.Fatal(err)
	}
}
...
func americanise(inFile io.Reader, outFile io.Writer) (err error) {
	...
	wordRx := regexp.MustCompile("[A-Za-z]+") eof := false
	for !eof {
		var line string
		line, err = reader.ReadString('\n') if err == io.EOF {
		err = nil // io.EOF isn't really an error
		eof = true // this will end the loop at the next iteration } else if err != nil {
